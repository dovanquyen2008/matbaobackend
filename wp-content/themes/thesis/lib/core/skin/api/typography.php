...
class thesis_skin_typography {
	...
	public function __construct() {
		...
		if (empty($thesis->environment) && !apply_filters('thesis_skin_typography_get_fonts_anyway', false)) return;
	...
	*/
	public function height($size, $width = false, $font = false) {
		$a = 1 / (2 * $this->phi);
		$factor = $this->cpl / (!empty($font) && !empty($this->fonts[$font]) && !empty($this->fonts[$font]['mu']) ? $this->fonts[$font]['mu'] : $this->mu);
