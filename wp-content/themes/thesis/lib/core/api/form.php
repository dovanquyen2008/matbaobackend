	...
	public function field($field, $id, $name, $value = false, $tabindex = 1, $depth = 0) {
		...
		if ($field['type'] == 'text') {
			...
			$output =
				...
				"$tab\t\t<input type=\"text\" class=\"text_input$class\" id=\"$id\" name=\"$name\" value=\"". esc_attr(stripslashes($value)). "\"$placeholder$req tabindex=\"$tabindex\" />\n".
				(!empty($field['counter']) ?
		...
		elseif ($field['type'] == 'email') {
			...
			$output =
				"$tab\t<p$classes>\n".
				"$tab\t\t<label for=\"$id\">{$field['label']}$label</label>\n".
				"$tab\t\t<input type=\"email\" class=\"email_input$class\" id=\"$id\" name=\"$name\" value=\"". esc_attr(stripslashes($value)). "\"$placeholder$req tabindex=\"$tabindex\" />\n".
				"$tab\t</p>\n";
		}
		elseif ($field['type'] == 'password') {
			...
			$output =
				"$tab\t<p$classes>\n".
				"$tab\t\t<label for=\"$id\">{$field['label']}$label</label>\n".
				"$tab\t\t<input type=\"password\" class=\"password_input$class\" id=\"$id\" name=\"$name\" value=\"". esc_attr(stripslashes($value)). "\"$placeholder$req tabindex=\"$tabindex\" />\n".
				"$tab\t</p>\n";
		}
		elseif ($field['type'] == 'hidden') {
			$output = "$tab<input type=\"hidden\" class=\"hidden_input\" id=\"$id\" name=\"$name\" value=\"". esc_attr(stripslashes($value)). "\" />\n";
		}
		...
		elseif ($field['type'] == 'textarea') {
			...
			$output =
				"$tab\t<p$classes>\n".
				"$tab\t\t<label for=\"$id\">{$field['label']}$label</label>\n".
				"$tab\t\t<textarea id=\"$id\"$class name=\"$name\"". (!empty($field['rows']) && is_numeric($field['rows']) ? " rows=\"{$field['rows']}\"" : ''). "$req tabindex=\"$tabindex\">". htmlspecialchars(stripslashes($value)). "</textarea>\n".
				(!empty($field['counter']) ?
				"$tab\t\t<input type=\"text\" readonly=\"readonly\" class=\"counter\" size=\"2\" maxlength=\"3\" value=\"0\">\n".
				"$tab\t\t<label class=\"counter_label\">{$field['counter']}</label>\n" : (!empty($field['description']) ?
				"$tab\t\t<span class=\"input_description\">{$field['description']}</span>\n" : '')).
		...
		elseif ($field['type'] == 'add_media') {
			...
			$output =
				...
				"$tab\t\t<button class=\"t_media_upload button\"><span class=\"wp-media-buttons-icon\"></span>$verb</button>\n".
				"$tab\t</p>\n".
